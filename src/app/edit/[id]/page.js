...
export default function Edit() {
  ...
  const { id } = useParams();  // ✅ useParams() — works in ALL Next.js versions
  ...
  const update = async () => {
    ...
    try {
      setLoading(true);
      await axios.put(`/api/articles/${id}`, { title, tag, content });
      router.push("/");
    } catch (err) {
      alert("Update failed");
      console.error(err);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0f1117", color: "#e2e8f0", fontFamily: "'Segoe UI', sans-serif" }}>
      <div style={{ background: "#161b27", borderBottom: "1px solid #2d3548", padding: "20px 40px", display: "flex", alignItems: "center", gap: "16px" }}>
        ...
        <h1 style={{ margin: 0, fontSize: "20px" }}>Edit Article</h1>
      </div>
